...
from controllers.elasticsearchcontroller import ElasticsearchController
...
class BookController(AmqpController, S3Controller, ElasticsearchController):
    ...
    async def callback(self, channel, body, envelope, properties):
        # raise Exception('howowo')
        body_dict = json.loads(body)
        ret = await self.create('index', 'doc', body_dict)
        logger.info(ret)
        logger.info(body_dict)
        logger.info('shahahahah-------------------------------')
